...
int j=0, idG[1000], vacinaG[1000], qtdCadastros, editar;
double alturaG[1000];
...
void Adicionar_editar_Usuario(){
  ...
  char repetir,repetir3,repetir4, repetir5;
  double altura;
...
char* validarSexo(char*sexoValido){
  ...
  do {
    ...
    fflush(stdin);
    gets(sexoValido);
    if ((strcmp(sexoValido,"Feminino")==0) || (strcmp(sexoValido,"Masculino")==0) || (strcmp(sexoValido,"Nao declarar")==0))
      break;
    else{
...
void buscarEmail() {
  ...
  do {
    ...
    if ((i==qtdCadastros)&&(repetir!='N'))
      printf("Email invalido");
    i++;
  } while((repetir!='N')&&(i<=qtdCadastros));
  printf("\n\nDigite 1 para sair!\n");
  scanf("%c", &sair);
  system("cls");
}
...
void excluirUsuario(){
  int i, j, l=0, idBusca, aux, sair, cont, excluidos=-1000;
  ...
  do {
    if (idBusca==idG[l]) {
      idG[l]=excluidos;
      strcpy(emailG[l],"Email excluido");
      for (j = 0; j < cont-1; j++){
        for (i = 0; i < cont-1; i++){
          if (idG[i] > idG[i+1]){
            aux = idG[i];
    ...
    l++;
  } while((repetir!='N')&&(l<qtdCadastros));
  if (repetir!='N') {
    printf("ID Invalido\n");
    printf("\nDigite 1 para sair!");
    scanf("%i", &sair);
